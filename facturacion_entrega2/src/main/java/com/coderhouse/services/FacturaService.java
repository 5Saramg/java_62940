...
public class FacturaService {
	...
	public Factura asignarProductos(productosEnFacturaDTO dto) {
		Factura factura = facturaRepository.findById(dto.getCodeFac())
				.orElseThrow(() -> new IllegalArgumentException("Factura no encontrado"));
		...
		factura.setTotal(calculaTotalFactura(factura.getProductos()));
	...
	public Factura asignarCliente(Long codeCliente, Long codeFac) {
		...
		Cliente cliente = clienteRepository.findById(codeCliente)
				.orElseThrow(() -> new IllegalArgumentException("Cliente no registrado"));
		Factura factura = facturaRepository.findById(codeFac)
				.orElseThrow(() -> new IllegalArgumentException("Factura no encontrado"));
	...
	public int calculaTotalFactura(List<Producto> productos) {
		int total = 0;
